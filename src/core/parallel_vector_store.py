...
try:
    ...
    # Handle FAISS import issues (binary incompatibility, missing dependencies, etc.)
    HAS_FAISS = False
    faiss = None
...
class ParallelVectorStore:
    ...
    def __init__(self, embedding_service: Union[EmbeddingService, None] = None, gpu_service: Union[GPUService, None] = None):
        ...
        self.gpu_index: Union[faiss.Index, None] = None
    ...
    async def initialize(self) -> dict[str, Any]:
        ...
        try:
            ...
            if not HAS_FAISS:
                logger.warning("⚠️ FAISS not available, running in fallback mode")
                return {
                    "status": "success",
                    "message": "Vector store running in fallback mode (no FAISS)",
                    "fallback_mode": True,
                    "features": ["basic_operations", "no_gpu_acceleration", "limited_search"]
                }
    ...
    async def add_documents(self, documents: list[VectorDocument], batch_size: Union[int, None] = None) -> dict[str, Any]:
        """
    ...
    async def search(
        self,
        query_embedding: Union[np.ndarray, None] = None,
