...
class BasicTextExtractor:
    ...
    def _check_dependencies(self):
        ...
        # Initialize EasyOCR if available and configured
        if HAS_EASYOCR and self.ocr_engine == "easyocr":
            try:
                self.easyocr_reader = easyocr.Reader(self.ocr_languages, gpu=False)  # CPU for compatibility
                logger.info("✅ EasyOCR reader initialized")
            except Exception as e:
                logger.warning(f"Failed to initialize EasyOCR: {str(e)}")
    
    async def extract_text(self, file_path: str, file_type: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def _extract_image_text(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.ocr_engine == "easyocr" and self.easyocr_reader:
                ...
                return {
                    "status": "error",
                    "error": f"OCR engine '{self.ocr_engine}' not available"
                }
